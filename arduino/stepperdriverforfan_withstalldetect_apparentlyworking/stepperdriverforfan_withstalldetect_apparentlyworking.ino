...
bool stall_detect(float x, float current) {
    // polynomial must end with a semicolon
    ...
    if ( (expected_current * 0.6f < current) &&
         (current < expected_current * 1.4f) ) 
    {
        return false;   // within expected range → NOT stalled
    }
    else {
        return true;    // outside expected range → stall
    }
}
...
void loop() {
 ...
 float Vbase = (22.0f * (RPS / rps_max)) + 2.5f;   // maps 0..rps_max -> 3..16 V
