...
current_min = expected_current * 0.3f;
current_max = expected_current * 1.2f + 0.15f;
  if ((current_min < current_filt) &&
      (current_filt < current_max)) {
    return false;
  } else {
    return true;
  }
...
void loop() {
  ...
  if (nowMs - lastPrintMs >= printIntervalMs) {
    lastPrintMs = nowMs;
   Serial.print("voltage: "); Serial.println(motor.voltage_limit); 
   Serial.print("voltage override: "); Serial.println(voltage_override);
   Serial.print("current: "); Serial.println(Iraw);
   Serial.print("expected_current2: "); Serial.println(expected_current2);
   Serial.print("current_min: "); Serial.println(current_min);
  Serial.print("current_max: "); Serial.println(current_max);
  if (digitalRead(use_pwm) == 0){
    Serial.print("status: "); Serial.println("using pwm as throttle input");
  }
    else {
  Serial.print("status: "); Serial.println("using potentiometer as throttle input");
  }
  }
  ...
  norm   = (float)raw / 4095.0f;      // 0..1
  }
 
  target = norm * rps_max;
  //logic to get it to turn off fully at lower power command levels, remember there is no stall detect below a certain rps so it's important not to allow operation in that region for long
  if (target < rps_min){
    target = 0;
  } 
   if (RPS<(HYST_RPS+0.10)){  
    voltage_override = 0.0f;
    started = false;
  }
else{
  if (started == false){
voltage_override = 1.0f;
started = true;
  }
